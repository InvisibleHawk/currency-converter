...
const currenciesDateEndpoint = async (date: string, currencyA: string, currencyB: string) => {
...
export const getHistoryCurrency = (valA: string, valB: string) => {
  ...
  return Promise.all(dates.map((date) => currenciesDateEndpoint(date, valA, valB)))
}
